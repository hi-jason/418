...
- Logistical Regression (R package glmnet)
- Logistical Regression with Lasso
- Logistical Regression (H20)
- Random Forest (R package xgboost)
- Gradient Boosting Machines (R package xgboost)
...
```
Logistical Regression with R package glmnet
...
```
```{r LRplot, echo=FALSE}
plot(performance(rocr_pred, "tpr", "fpr"), main="ROC Curve - Logistical Regression (Validation)")
...
```
Lambda is very low which is why when the model was set to the minimum of lambda it was roughly the equivalent of logistical regression with no lambda.  It seems in the validation set, that no lambda was slightly better but just slight difference (0.00004).


```{r plot of lambda, echo=FALSE}
plot(performance(rocr_predlrmd3, "tpr", "fpr"), main="ROC Curve - Logistical Regression (Val) w/Lambda")
